...
<script>
        document.addEventListener("DOMContentLoaded", () => {
            // Fetch and render agents count prediction
            fetch('/predictTotalAgents')
                .then(response => response.json())
                .then(data => {
                    const monthsYears = data.map(item => `${getMonthName(item.month)} ${item.year}`);
                    const agentCounts = data.map(item => item.agent_count);
            ...
            // Fetch and render applicants count prediction
            fetch('/predictTotalApplicants')
                .then(response => response.json())
                .then(data => {
                    const monthsYears = data.map(item => `${getMonthName(item.month)} ${item.year}`);
                    const applicantCounts = data.map(item => item.applicant_count);
            ...
            // Fetch and render commissions prediction
            fetch('/predictTotalCommissions')
                .then(response => response.json())
                .then(data => {
                    const monthsYears = data.map(item => `${getMonthName(item.month)} ${item.year}`);
                    const commissionAmounts = data.map(item => item.total_commission);

                    new ApexCharts(document.querySelector("#commissionChart"), {
                        series: [{
